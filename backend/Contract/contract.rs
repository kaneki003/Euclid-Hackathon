...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::PlaceBet { amount, player } => place_bet(deps, amount, player, info.sender),
...
fn resolve_game(
    ...
    if let Some(mut session) = state.sessions.get_mut(&player).cloned() {
        if session.is_active {
            ...
            if random_number % 2 == 0 {
                ...
                // call the function to take the bet amount from the player
    ...
    Ok(Response::new().add_attribute("action", "no_active_session found for the user"))
}
...
fn query_active_sessions(deps: Deps) -> StdResult<Vec<GameSession>> {
    ...
    let active_sessions: Vec<GameSession> = state
        .sessions
        .values()
